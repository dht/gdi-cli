...
const getPackages = (cwd, patterns, extraPatterns) => {
    ...
    return globby
        ...
        .map((packagePath) => {
            ...
            const dependenciesGdi = _.pickBy(dependencies, (value, key) =>
                key.match(/^@gdi/)
            );
...
const setPackageDependenciesVersion = (cwd, packageNames, version) => {
    const packageJson = path.resolve(cwd, 'package.json');
    const contentRaw = fs.readFileSync(packageJson).toString();
    const content = JSON.parse(contentRaw);
    ...
    packageNames.forEach((packageName) => {
        if (content.dependencies[packageName]) {
            content.dependencies[packageName] = '^' + version;
        }

        if (content.peerDependencies && content.peerDependencies[packageName]) {
            content.peerDependencies[packageName] = '^' + version;
        }

        if (content.devDependencies && content.devDependencies[packageName]) {
            content.devDependencies[packageName] = '^' + version;
        }
    });
