...
const writeLatestVersionForAllPackages = async () => {
    const output = {};
    allPackages = await getInfo();

    console.time('getLatestVersion');

    const promises = allPackages.map((packageInfo) =>
        getLatestVersionInNpm(packageInfo.name)
    );
...
const addCurrentStoreVersion = () => {
    const storeVersions = fs.readFileSync(
        '.cloc/.dependency.versions.json',
        'utf8'
    );
...
const getInfo = async () => {
    ...
    workspaces = workspaces.filter(
        (workspace) => !workspace.includes('submodules/')
    );
    ...
    const packages = await globby(packageJsonFiles, {
        cwd: root,
    });
