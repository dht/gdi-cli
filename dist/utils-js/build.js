...
const build = (cwd, shouldWriteLogs) => __awaiter(void 0, void 0, void 0, function* () {
    const raw = yield exec('npm', ['run', 'build', '--no-color'], cwd);
    ...
    if (shouldWriteLogs) {
        ...
        fs.writeFileSync(logsDir + '/build.stats.log', JSON.stringify(stats, null, 4));
        fs.writeFileSync(logsDir + '/build.errors.log', JSON.stringify(errorsByFile, null, 4));
    }
    return {
        raw,
        stats,
...
const parseDuration = (raw) => {
    let duration = 0;
    try {
        const durationRegex = /\[vite:dts\] Declaration files built in (\d+)ms\./g;
...
const parseBundleSizes = (raw) => {
    ...
    const regexEs = /^dist\/[a-z-]+\.es\.js\s+([0-9,\.]+) ([a-z]+) \/ gzip: *([0-9,\.]+) ([a-z]+)/gim;
    const regexEsMap = /^dist\/[a-z-]+\.es\.js\.map\s+([0-9,\.]+) ([a-z]+)/gim;
    const regexUmd = /^dist\/[a-z-]+\.umd\.js\s+([0-9,\.]+) ([a-z]+) \/ gzip: *([0-9,\.]+) ([a-z]+)/gim;
    const regexUmdMap = /^dist\/[a-z-]+\.umd\.js\.map\s+([0-9,\.]+) ([a-z]+)/gim;
    ...
    if (matchesEs.length > 0) {
        ...
        output.esSize = kibToBytes(parseFloat(size.replace(/,/g, '')) + sizeUnit);
        output.esSizeGzip = kibToBytes(parseFloat(gzipSize.replace(/,/g, '')) + gzipSizeUnit);
    }
    if (matchesEsMap.length > 0) {
        const [, size, sizeUnit] = matchesEsMap[0];
        output.esMapSize = kibToBytes(parseFloat(size.replace(/,/g, '')) + sizeUnit);
    }
    if (matchesUmd.length > 0) {
        const [, size, sizeUnit, gzipSize, gzipSizeUnit] = matchesUmd[0];
        output.umdSize = kibToBytes(parseFloat(size.replace(/,/g, '')) + sizeUnit);
        output.umdSizeGzip = kibToBytes(parseFloat(gzipSize.replace(/,/g, '')) + gzipSizeUnit);
    }
    if (matchesUmdMap.length > 0) {
        const [, size, sizeUnit] = matchesUmdMap[0];
        output.umdMapSize = kibToBytes(parseFloat(size.replace(/,/g, '')) + sizeUnit);
    }
